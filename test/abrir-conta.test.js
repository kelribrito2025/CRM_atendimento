...
test('abrir conta: manda quem abriu e o motivo, e devolve o link de uso único', async () => {
  ...
  try {
    const r = await s.chamar(`/api/conversas/${s.conversaId}/abrir-conta`, 'POST', { motivo: 'cliente relatou saldo divergente' });
    ...
    assert.equal(pedido.corpo.motivo, 'cliente relatou saldo divergente');
    ...
    assert.match(nota.texto, /Abriu a conta do cliente no site\. Motivo: cliente relatou saldo divergente/);
...
test('abrir conta: o navegador não escolhe quem está abrindo', async () => {
  ...
  try {
    await s.chamar(`/api/conversas/${s.conversaId}/abrir-conta`, 'POST', {
      motivo: 'conferir o saldo do cliente',
...
test('abrir conta: motivo curto, cliente sem site e site fora do ar', async () => {
  const s = await subirServidor();
  try {
    assert.equal((await s.chamar(`/api/conversas/${s.conversaId}/abrir-conta`, 'POST', { motivo: 'ver' })).status, 400);
    assert.equal((await s.chamar(`/api/conversas/${s.conversaId}/abrir-conta`, 'POST', {})).status, 400);
    assert.equal(s.site.pedidos.length, 0, 'nem chega a falar com o site');
    ...
    const semSite = await s.chamar(`/api/conversas/${cv.id}/abrir-conta`, 'POST', { motivo: 'conferir o saldo dele' });
...
test('abrir conta: recados claros quando o site recusa', async () => {
  for (const [status, trecho] of [[401, /chave de acesso/i], [403, /chave de acesso/i], [404, /não existe mais/i], [500, /respondeu com erro/i]]) {
    ...
    try {
      const r = await s.chamar(`/api/conversas/${s.conversaId}/abrir-conta`, 'POST', { motivo: 'conferir o saldo do cliente' });
