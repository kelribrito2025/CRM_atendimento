...
test('fundo login UI: ondas no login e em escolher-atendente (desktop), atrás dos textos', () => {
  const ler=p=>fs.readFileSync(path.join(__dirname,'..',p),'utf8');
  const css=ler('client/assets/css/acesso.css');
  const paginas='body:is\\(\\[data-pagina="login"\\], \\[data-pagina="escolher-atendente"\\]\\) \\.hero';
  assert.match(css,new RegExp(`${paginas} \\{ isolation: isolate; \\}`));
  const camada=css.match(new RegExp(`${paginas}::before \\{([^}]+)\\}`))?.[1];
  assert.ok(camada);
  ...
  assert.match(css,new RegExp(`@media \\(min-width: 861px\\) \\{\\s+${paginas}::before`));
