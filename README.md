...
No Chat do site, o cliente também pode **gravar um áudio** pelo microfone e enviá-lo como mensagem (chega no CRM com player; depende do S3 configurado). No Telegram e no Chat do site, uma resposta de texto já entregue pode ser **editada** (lápis ao passar o mouse na mensagem): no Telegram o bot troca o texto no chat do cliente, no site o chat recarrega na hora; nos dois o cliente vê a mensagem marcada como editada e o CRM registra a alteração na auditoria. Só quem enviou (ou um administrador) pode editar. No WhatsApp ainda não. No Telegram, a foto de perfil do cliente aparece no avatar (o CRM confere uma vez por dia se mudou). No WhatsApp, a foto aparece quando o servidor do uazapi manda o endereço dela no aviso da mensagem; a API oficial deles não tem um jeito de pedir a foto de um contato. Fotos, áudios, vídeos e documentos aparecem dentro da conversa (o CRM baixa o arquivo com o token do bot, que nunca vai para o navegador). No WhatsApp, mídias ainda aparecem só como texto, por exemplo "[Imagem]".
...
| `SENHA_PERFIL_ADMIN` | Senha pedida ao escolher um perfil de administrador na tela "Escolha seu atendente" (em branco: senha padrão) |
