...
async function principal() {
  ...
  if (args.listar) {
    const lista = await acesso.listarConvitesPendentes(db);
  ...
  const existente = await db.prepare('SELECT id FROM usuarios WHERE email = ? AND ativo = 1').get(email);
  if (existente) sair('Já existe uma conta ativa com este e-mail. Para trocar a senha use: npm run usuario -- --email ... --senha ...');

  const equipeIds = [];
  if (args.equipes) {
    const cadastradas = await db.prepare('SELECT id, nome FROM equipes ORDER BY ordem, nome').all();
  ...
  const admin = await db.prepare("SELECT id FROM usuarios WHERE papel = 'admin' AND ativo = 1 ORDER BY id LIMIT 1").get();
  const { token, expira } = await acesso.criarConvite(db, {
    email,
    papel: args.papel,
    equipeIds,
